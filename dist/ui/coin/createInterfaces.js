...
export async function genFavouritesMenu(interaction) {
    let selectFavs = new StringSelectMenuBuilder().setCustomId("favCoins").setPlaceholder("Quickly access your favourites...");
    ...
    for(let i = 0; i < favs.length; i++){
        ...
        if (favName) {
            selectFavs = selectFavs.addOptions({
                label: favName.name,
...
export function genCoinEmbed(choice, client) {
    return getEmbedTemplate(client).setThumbnail(`https://s2.coinmarketcap.com/static/img/coins/128x128/${choice.id}.png`).setColor(choice.percent_change_24h < 0 ? 0xed4245 : 0x3ba55c).setTitle(`${choice.name} (${choice.symbol}-USD)`).setURL(`https://coinmarketcap.com/currencies/${choice.slug}`).setFields({
        ...
        name: "Last Updated",
        value: `<t:${new Date(choice.last_updated).getTime}:R>`
    });
}
