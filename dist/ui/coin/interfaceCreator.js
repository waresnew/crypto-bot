...
export async function makeFavouritesMenu(interaction) {
    const selectFavs = new StringSelectMenuBuilder().setCustomId(`coin_favCoins_${interaction.user.id}`).setPlaceholder("Quickly access your favourites...");
    ...
    for(let i = 0; i < favs.length; i++){
        ...
        if (favName) {
            selectFavs.addOptions({
                label: favName.name,
                value: row.favouriteCrypto.toString()
            });
        }
    }
    if (selectFavs.options.length == 0) {
        selectFavs.addOptions({
            label: "Favourite a coin to add it here!",
            value: "default"
        });
    }
    return new ActionRowBuilder().addComponents(selectFavs);
}
export function makeEmbed(choice, client) {
    return getEmbedTemplate(client).setThumbnail(`https://s2.coinmarketcap.com/static/img/coins/128x128/${choice.id}.png`).setColor(choice.percent_change_24h < 0 ? 0xed4245 : 0x3ba55c).setTitle(`${choice.name} (${choice.symbol}-USD)`).setURL(`https://coinmarketcap.com/currencies/${choice.slug}`).setFields({
        name: "Price",
...
export async function makeButtons(choice, interaction) {
    ...
    return new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId(`coin_alerts_${interaction.user.id}`).setLabel("Add alert").setEmoji("🔔").setStyle(ButtonStyle.Primary)).addComponents(new ButtonBuilder().setCustomId(`coin_setfav_${interaction.user.id}`).setLabel(favourited ? "Unfavourite" : "Favourite").setEmoji("⭐").setStyle(favourited ? ButtonStyle.Secondary : ButtonStyle.Primary)).addComponents(new ButtonBuilder().setCustomId(`coin_refresh_${interaction.user.id}`).setLabel("Refresh").setEmoji("🔄").setStyle(ButtonStyle.Primary));
}
