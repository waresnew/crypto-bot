import { chatInputApplicationCommandMention, SlashCommandBuilder, ActionRowBuilder, ButtonBuilder, ButtonStyle, ComponentType } from "discord.js";
...
export default {
    ...
    async execute (interaction) {
        ...
        let choice = await db.get("select * from cmc_cache where symbol=? collate nocase or name=? collate nocase", input);
        if (!choice) {
            const suggestion = didyoumean(input.toLowerCase(), cryptoSymbolList.concat(cryptoNameList));
            interaction.reply(`Couldn't find a coin called \`${input}\`. ${suggestion != null ? `Did you mean ${chatInputApplicationCommandMention(interaction.commandName, interaction.commandId)} \`${suggestion}\`?` : ""}`);
            return;
        }
        ...
        const buttons = new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId("alerts").setLabel("Add Alert").setEmoji("🔔").setStyle(ButtonStyle.Primary)).addComponents(new ButtonBuilder().setCustomId("setfav").setLabel(favourited ? "Unfavourite" : "Favourite").setEmoji("⭐").setStyle(favourited ? ButtonStyle.Secondary : ButtonStyle.Primary));
        ...
        const favouriteCollector = response.createMessageComponentCollector({
        ...
        favouriteCollector.on("collect", async (i)=>{
            ...
            choice = await db.get("select * from cmc_cache where id=?", selected);
        ...
        buttonCollector.on("end", async (_collected)=>{
            ...
            interaction.editReply({
                content: "This message is no longer receiving input.",
                embeds: [
                    embed
                ],
                components: [
