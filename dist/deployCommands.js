...
for (const file1 of devCommandFiles){
    const command1 = (await import(path.join(pathToFileURL(devCommandPath).toString(), file1))).default;
    devCommands.push(command1.data.toJSON());
}
...
async function registerCommands(commands, dev) {
    ...
    try {
        ...
        await rest.put(dev ? Routes.applicationGuildCommands(process.env["APP_ID"], process.env["GUILD_ID"]) : Routes.applicationCommands(process.env["APP_ID"]), {
            body: []
        });
