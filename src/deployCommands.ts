...
for (const file of commandFiles) {
  const command = (
    await import(path.join(pathToFileURL(commandPath).toString(), file))
  ).default;
  commands.push(command.data.toJSON());
}

const devCommands: Command[] = [];
const devCommandPath = path.join(
  path.dirname(fileURLToPath(import.meta.url)),
  "commands",
  "dev"
);
...
for (const file of devCommandFiles) {
  const command = (
    await import(path.join(pathToFileURL(devCommandPath).toString(), file))
  ).default;
